...
class encoder(nn.Module):
    ...
    def __init__(self, n_classes,
        ...
        self.predict = nn.Sequential(
            nn.Linear(readout_feats * 2 + n_classes, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, latent_feats * 2)
        )   

    def forward(self, vector, graph_embed):

        mu, log_var = torch.split(self.predict(torch.cat([vector, graph_embed], 1)), [self.latent_feats, self.latent_feats], dim = 1)
...
class decoder(nn.Module):  
    ...
    def __init__(self, n_classes,
        ...
        self.predict = nn.Sequential(
            nn.Linear(readout_feats * 2 + latent_feats, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, predict_hidden_feats), nn.PReLU(),
            nn.Linear(predict_hidden_feats, n_classes)
        )   

    def forward(self, latent, graph_embed):

        return self.predict(torch.cat([latent, graph_embed], 1))
...
class VAE(nn.Module):
    ...
    def forward(self, rmols, pmols, vector):

        graph_embed = self.rmpnn(rmols, pmols)
        
        mu, log_var = self.encoder(vector, graph_embed)
        std = torch.exp(0.5 * log_var)
        eps = torch.randn_like(mu)
        latent = mu + eps * std
        
        vector_pred = self.decoder(latent, graph_embed)

        return vector_pred, mu, log_var
    
    def sampling(self, rmols, pmols):
        
        graph_embed = self.rmpnn(rmols, pmols)
        latent = torch.randn(len(graph_embed), self.latent_feats).cuda()
...
class Trainer:
    ...
    def inference(self, tst_loader, n_sampling = 100):
    
        def forward(tst_loader):
            ...
            with torch.no_grad():
                for batchidx, batchdata in enumerate(tst_loader):
                    ...
                    preds_list = self.net.sampling(inputs_rmol, inputs_pmol).cpu().numpy()
        ...
        self.net.eval()   
        tst_y_preds_list = [forward(tst_loader) for _ in range(n_sampling)]
        tst_y_preds = [sum([tst_y_preds_list[j][i] for j in range(n_sampling)], []) for i in range(len(tst_y_preds_list[0]))]
